  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setNotice({ kind: "error", text: `${result.error || "整理失败，请稍后重试。"}${result.saved && source ? ` 原文文件：DATA_DIR/raw/${source}` : ""}` });
        return;
      }
      setRaw("");
    ...
    <div className="grid items-start gap-7 lg:grid-cols-[minmax(0,1.35fr)_minmax(300px,.65fr)]">
      <section className="rounded-2xl border border-[#dce5d5] bg-white p-5 shadow-[0_3px_0_#e9ede1] sm:p-7">
        ...
        <form onSubmit={save}>
          ...
          <div className="mt-2 flex justify-between text-[10px] text-[#97a28f]"><span>整理后的原文和摘要对访客公开，请勿提交敏感内容</span><span>{raw.length.toLocaleString()} / 100,000</span></div>
      ...
      <aside className="space-y-5">
        <div className="rounded-2xl border border-[#dce5d5] bg-[#eef2e8] p-6"><p className="eyebrow"><span/> HOW IT WORKS</p><h2 className="mt-4 text-base font-semibold text-[#3d5738]">从原文到可检索的札记</h2><ol className="mt-5 space-y-4 text-xs leading-6 text-[#75866c]"><li><strong className="mr-3 text-[#98aa83]">01</strong> 保存原文，整理后公开展示</li><li><strong className="mr-3 text-[#98aa83]">02</strong> {demo ? "演示规则" : "AI 模型"}提取摘要、标签与题目</li><li><strong className="mr-3 text-[#98aa83]">03</strong> 切分内容，生成搜索向量</li></ol><p className="mt-5 border-t border-[#dbe5d5] pt-4 text-[10px] leading-5 text-[#94a18b]">{demo ? "当前使用演示处理。配置真实模型后可获得 AI 分析与语义向量。" : "整理可能需要一些时间；原文会一直保留，供你核对分析结果。"}</p></div>
